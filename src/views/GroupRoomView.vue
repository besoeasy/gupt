...
const showMentionBar = computed(() => !!lastMentionId.value && !mentionDismissed.value);
...
watch(
  ...
  (rows, previousRows = []) => {
    ...
    onMessagesUpdated(rows, previousRows, {
      loadingOlder: loadingOlder.value,
      selfPubkey: selfPubkey.value,
    });
  },
  { immediate: true },
);
...
async function loadOlderMessages() {
  ...
  try {
    ...
    await messenger.refreshGroupFromDexie(groupId.value);
  } catch (e) {
...
watch(
  ...
  (id) => {
    ...
    if (id) void messenger.markGroupSeen(id);
  },
  { immediate: true },
);
...
<template>
  <div class="relative flex flex-col h-full bg-(--app-bg) text-(--app-text)">
    <!-- Chat header -->
    <div
      class="px-3 py-3 sm:px-4 md:px-5 flex items-center justify-between gap-3 shrink-0 border-b border-(--app-border) bg-[color-mix(in_srgb,var(--app-bg)_82%,transparent)] ]"
    >
      <div class="flex items-center gap-3 min-w-0">
        <button
          @click="router.push('/')"
          class="inline-flex items-center justify-center rounded-2xl border border-(--app-border) bg-(--app-surface-soft) text-(--app-text-soft) hover:border-(--app-border-strong) hover:bg-(--app-surface-hover) hover:text-(--app-text) h-10 w-10 shrink-0"
          title="Back"
        >
    ...
    <div class="flex flex-1 min-h-0">
      ...
      >
        <aside
          v-if="drawerOpen"
          class="w-[280px] max-lg:fixed max-lg:inset-y-0 max-lg:right-0 max-lg:z-50 max-lg:w-[min(320px,85vw)] xl:w-[300px] bg-[color-mix(in_srgb,var(--app-bg)_82%,transparent)] ] shrink-0 border-l border-white/7 overflow-y-auto"
        >
          <!-- Drawer header -->
          <div
