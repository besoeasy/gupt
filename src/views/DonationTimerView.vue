...
<template>
  <div class="min-h-screen bg-(--app-bg) text-(--app-text) flex flex-col items-center justify-center p-6 sm:p-10 md:p-12">
    ...
    >
      ...
      <div class="space-y-4">
        <h2 class="text-xs font-bold text-zinc-400 uppercase tracking-widest text-center">Select Donation Method</h2>

        <div class="space-y-4">
          <!-- Option 1: GitHub Sponsors (UP) -->
          <div class="rounded-2xl border border-(--app-border) bg-(--app-surface-soft) p-5 space-y-3">
          ...
          <div class="relative flex items-center justify-center py-1">
            ...
            <div class="relative rounded-full bg-(--app-surface) px-4 py-1 border border-rose-500/30 text-rose-400 font-black text-xs uppercase tracking-widest">
              VS
            </div>
          </div>
        ...
        <div v-if="showQr && fundingAddress" class="rounded-2xl border border-(--app-border) bg-(--app-surface-soft) p-5 flex flex-col items-center space-y-3">
          <div class="p-3 bg-white rounded-2xl shadow-sm border border-zinc-200">
            <canvas ref="qrCanvas" class="block rounded-xl" />
          </div>
          <p class="font-mono text-xs text-(--app-text) break-all text-center max-w-xs bg-(--app-surface) px-4 py-2 rounded-xl border border-(--app-border)">
            {{ fundingAddress }}
          </p>
        </div>
      </div>
