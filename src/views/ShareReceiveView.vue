        ...
        <div v-else-if="payload" class="space-y-8">
          ...
          <section v-if="payload.media?.length" class="space-y-4">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              ...
              <button
                type="button"
                class="inline-flex items-center justify-center gap-2 rounded-[14px] border border-(--app-border) bg-(--app-surface-soft) px-4 py-2 text-sm font-bold text-(--app-text) transition-all duration-200 hover:-translate-y-0.5 hover:bg-(--app-surface-hover) active:scale-[0.97] focus:outline-none focus-visible:ring-2 focus-visible:ring-[color-mix(in_srgb,var(--app-primary)_60%,transparent)] h-9 !px-3 !text-xs"
            ...
            <div class="space-y-3">
              <div v-for="(file, idx) in payload.media" :key="idx" class="space-y-2">
                <div
                  class="overflow-hidden rounded-2xl border border-(--app-border) bg-(--app-surface-soft) p-4"
                >
