...
export async function publish(event, peerPubkey = null, options = {}) {
  ...
  console.log("[gupt-relay-publish] base relays", {
    count: relays.length,
  ...
  if (peerPubkey) {
    ...
    console.log("[gupt-relay-publish] with peer hints", {
      totalRelays: relays.length,
      hintCount: hintUrls.length,
  ...
  console.log("[gupt-relay-publish] result", {
    published: publishedRelays.length,
    failed: failedRelays.length,
    failedDetail: failedRelays.slice(0, 3),
  });
