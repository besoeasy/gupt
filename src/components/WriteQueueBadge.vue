...
import { getSendQueueSnapshot } from "@/lib/sendQueue";
...
<template>
  ...
  >
    ...
    >
      <div
        class="mb-3 flex items-center gap-2 rounded-full border border-(--app-border) bg-(--app-surface)/90 px-4 py-2 text-xs font-medium text-(--app-text-soft) shadow-lg backdrop-blur-md"
      >
        <!-- Pulsing dot -->
        <span class="relative flex h-2 w-2 shrink-0">
          <span class="absolute inline-flex h-full w-full animate-ping rounded-full bg-(--app-primary) opacity-60" />
